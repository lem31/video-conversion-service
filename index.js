...
async function downloadVideoWithYtdlpUltimate(videoUrl, outputDir, isPremium) {
  ...
  try {
    ...
    // NOTE: external segmented downloader support (aria2) removed for Railway compatibility.
    // If you want to enable a permitted external downloader later, add it here.
    ...
    if (enablePipe) {
      ...
      // Forward cobalt external-downloader args to the piped invocation when enabled
      if (process.env.YTDLP_USE_COBALT === '1') {
        const cobaltName = process.env.YTDLP_COBALT_NAME || 'cobalt';
        const cobaltArgsRaw = process.env.YTDLP_COBALT_ARGS || '--parallel 8';
