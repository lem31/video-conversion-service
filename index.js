...
async function downloadVideoWithYtdlp(videoUrl, outputDir) {
  ...
  try {
    ...
    // First, try to get video info to validate the URL
    ...
    console.log('Video info retrieved:', info.title, info.duration);
  ...
  try {
    if (req.file) {
      ...
      if (isSupportedVideoUrl(videoUrl)) {
        ...
        try {
          ...
          const errorCode = errorMsg.split(':')[0]; // Get error code before colon
    ...
    ffmpeg(inputPath)
      ...
      .on('end', () => {
        ...
        // Return base64 audio data in the format expected by Next.js API
        res.json({
          success: true,
